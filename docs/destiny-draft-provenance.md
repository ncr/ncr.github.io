...
## Mystery montage and musical entrance

The author clarified that the first ~20 seconds must show different objects. The opening now stays close throughout four successive passages: Fusion Transport 0–5.85s, Proxy 5.85–11.31s, Quantum Simulator 11.31–16.963s and Sky Racer from 16.963s. Camera rigs start already beside the active nib, rather than beginning with a silhouette/establishing view. The three initial passages have no retreat or overview. Cold-load and effects-off image fallbacks are also cropped to genuine details. The header withholds device names until the reveal, and captions no longer identify the ship in advance.

A continuous retreat starts on the measured bass attack at 20.443s. The registered handoff finishes into the first full sheet at 22.203s, followed by its slow overview retreat. Quantum gets its full drawing/reveal at 23.85s; later stories retain their order. The complete 299.21-second show still includes all 42 wallpapers, with 45 drawing passages. `score-camera-passages.py` preserves explicitly authored intro timings; `prepare-film.mjs` also exports the close image-fallback crops.

Intro transitions blend two actual close wireframe views using GPU render targets, rather than inserting an outgoing full-sheet image. The outgoing pose is rendered once at its deterministic endpoint; the incoming view keeps moving during the .85-second blend. There is no screenshot readback or second continuously simulated scene. Targets use 2-sample antialiasing and are released after the intro.

Bass particles are held back until 20.443s. After the entrance, the same measured bass attacks add small smooth camera push/bank accents: 65 ms eased attack and 315 ms release, over the original damped contour-following movement. The accents are added after spring integration to avoid a delayed response and taper out during the registered retreat. They are baked into the 90 Hz camera tables; native overview frames retain their slow outward movement. Six independent laser-head bands and the real wireframe nozzle interlude remain.
