...
score=json.loads((root/'site/src/data/drawing-score.json').read_text());tour=json.loads((root/'site/src/data/destiny-tour.json').read_text());assert len({s['id'] for s in tour})==42
try:
 with sync_playwright() as p:
  ...
  for at in [5.85,11.31,16.963]:
   seek(round(at+.425,3));page.wait_for_function('document.querySelector(".viewer-canvas").dataset.liveBlend==="1"');assert not page.locator('.scene-bridge').is_visible()
   assert .48<float(page.locator('.viewer-canvas').get_attribute('data-intro-blend'))<.52
  seek(21.8);assert page.locator('.fusion-flight').evaluate('(e)=>Number(e.dataset.dissolve)>.5&&Number(e.style.opacity)>0')
  seek(22.25);assert page.locator('.fusion-flight').evaluate('(e)=>e.dataset.phase==="hold"&&Number(e.style.opacity)===0');assert page.locator('.viewer-title').inner_text()=='Sky Racer'
  ...
  page.wait_for_function('!document.querySelector(".tour-music").paused&&document.querySelector(".tour-music").currentTime>23.3',timeout=60000);assert page.evaluate('window.pauses')==0
  ...
  assert not errors,errors;browser.close();print('PASS: four mystery objects, three GPU close-up dissolves, no overview before 20.44s, full reveal at 22.20s, uninterrupted actual MP3 through entrance, mobile, 42 sheets, noindex.')
finally:
