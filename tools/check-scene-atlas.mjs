...
for(const [i,b]of data.bridges.entries()){
 ...
 for(const aspect of [.78,1,1440/407,2.4]){
  ...
  for(let t=b.beats[0];t<b.beats[4];t+=.01){const p=pose(t);for(const k of ['x','y','height','roll','opacity'])assert(Number.isFinite(p[k]));assert(p.height>0);assert(p.opacity>=-1e-10&&p.opacity<=1+1e-10);
...
assert(!shader.includes('attribute float spark'));assert(!shader.includes('spark>=0.'));assert(shader.includes('length:26'));assert(shader.includes('gl_PointSize=1.12+.18*pulse'));
console.log('PASS: 42 unique animated tiles, 31 cardinal four-beat bridges, continuous camera at four aspect ratios, finite precomputed contours, 26 line-width nibs and no spray geometry.');
