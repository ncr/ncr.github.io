...
try:
 with sync_playwright() as p:
  ...
  for i in range(8,16):
   ...
   if i>=14:assert peak['scale']>float(page.locator('.viewer-canvas').get_attribute('data-paper-scale'))*1.02
  assert rolls[2]>15 and rolls[2]>rolls[3]>rolls[4]>rolls[5]>rolls[6] and rolls[6]==0,rolls
  ...
  angles=[]
  for i in [1,4,6,8,9,10,11,12,13,14]:
   ...
   if i==13:assert .4<state['fade']<.6
   if i>=14:assert state['opacity']<.00001
  ...
  assert not errors,errors;browser.close();print('PASS: all eight final-half beats accent, 16-degree bank settles on beat 14, full-frame pulses, dinner 3D orbit and registered dissolve, mobile and reduced motion.')
finally:
