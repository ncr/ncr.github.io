...
function publish(){window.dispatchEvent(new CustomEvent('destiny-audio',{detail:{...bands}}));}
...
function reset(){cancelAnimationFrame(tickId);tickId=0;for(const key of ['bass','mids','highs','level'])bands[key]=0;bands.active=false;document.querySelectorAll('.music-meter i').forEach(el=>el.style.height='3px');publish();}
function tick(now){
 ...
 if(analyser&&context.state==='running'&&now-previous>=1000/30){
  ...
  const meter=document.querySelectorAll('.music-meter i');[bands.bass,bands.mids,bands.highs,bands.level].forEach((v,i)=>{if(meter[i])meter[i].style.height=(reduced.matches?(audible?8:3):3+v*15)+'px';});
  publish();
 }
